...
- 원칙 1 (Declarative by Default) — `ButtonGroup`, `ToggleGroup`, `InputGroup`의 기반
...
- `ScrollArea.Scrollbar` — 커스텀 스크롤바. `asChild`로 교체 가능
...
- `Tag.CloseButton` — 제거 버튼. `asChild`로 교체 가능
...
<Tag>
  React
  <Tag.CloseButton asChild>
    <button aria-label="제거"><MyCloseIcon /></button>
  </Tag.CloseButton>
</Tag>
...
- `TextField.Input` — pivot sentinel. input의 렌더 위치를 선언
- `TextField.Input` 이전 children → leading 영역 (아이콘, prefix 등)
- `TextField.Input` 이후 children → trailing 영역 (clear 버튼, suffix 등)
- 유효성 상태(`invalid` 등)는 `Field`가 담당 — TextField는 시각만 책임
- `TextField.Input` 누락 시 dev-time 경고 (원칙 16)
...
- `TextArea.Input` — textarea의 렌더 위치를 선언하는 pivot sentinel
- wrapper가 `flex-col`로 동작 — `TextArea.Input` 위아래에 자유롭게 content 배치 가능
...
- `color`: accent 색상 시스템
- `size`: `sm` | `md` | `lg`
- `Checkbox.Indicator` — 체크 / indeterminate 아이콘. `asChild`로 교체 가능
...
- 숨김 native input + 커스텀 UI (원칙 9)
- `Switch.Thumb` — 슬라이딩 thumb. `asChild`로 교체 가능
...
- `color`: accent 색상 시스템
- `size`: `sm` | `md` | `lg`
- `Radio.Indicator` — 선택 dot. `asChild`로 교체 가능
...
- `NumberField.Increment` — 증가 버튼. `asChild`로 교체 가능
- `NumberField.Decrement` — 감소 버튼. `asChild`로 교체 가능
...
<NumberField defaultValue={0}>
  <NumberField.Decrement asChild>
    <button><MinusIcon /></button>
  </NumberField.Decrement>
  <TextField.Input />
  <NumberField.Increment asChild>
...
- `TelField.CountrySelect` — 국기 + 국가코드 드롭다운 버튼. `asChild`로 교체 가능
...
- `Slider.Track` — 슬라이더 트랙. `asChild`로 교체 가능
- `Slider.Thumb` — 드래그 thumb. `asChild`로 교체 가능

```tsx
// 기본 사용 — Track / Thumb 자동 렌더
...
- `Progress.Indicator` — 채워진 바. `asChild`로 교체 가능
...
- 키보드 탐색 지원 (원칙 12)
- `Calendar.Header` — 월/연도 표시 헤더 영역. `asChild`로 교체 가능
- `Calendar.PrevButton` — 이전 달 이동 버튼. `asChild`로 교체 가능
- `Calendar.NextButton` — 다음 달 이동 버튼. `asChild`로 교체 가능
- `Calendar.Grid` — 날짜 그리드 영역. `asChild`로 교체 가능
- `Calendar.Day` — 개별 날짜 셀. `asChild`로 교체 가능

```tsx
// 기본 사용 — 모든 서브컴포넌트 자동 렌더
...
<Calendar mode="single" value={date} onChange={setDate}>
  <Calendar.Header>
    <Calendar.PrevButton asChild><button>←</button></Calendar.PrevButton>
    <span>{monthLabel}</span>
    <Calendar.NextButton asChild><button>→</button></Calendar.NextButton>
  </Calendar.Header>
  <Calendar.Grid>
    <Calendar.Day asChild>{(day) => <div className="custom-day">{day}</div>}</Calendar.Day>
  </Calendar.Grid>
</Calendar>
```
...
- `value` / `defaultValue` / `onChange` (원칙 15)
- `DatePicker.Trigger` — TextField 형태 트리거 영역. `asChild`로 교체 가능
- `DatePicker.Calendar` — 부동 패널 안의 캘린더 영역. `asChild`로 교체 가능
- `TimePicker.Trigger` — TextField 형태 트리거 영역. `asChild`로 교체 가능
- `TimePicker.Column` — 시/분/초 스크롤 컬럼. `asChild`로 교체 가능
...
<DatePicker value={date} onChange={setDate}>
  <DatePicker.Trigger asChild>
    <button className="custom-trigger">{date ?? '날짜 선택'}</button>
  </DatePicker.Trigger>
  <DatePicker.Calendar />
</DatePicker>
...
- `value` / `defaultValue` / `onChange` (원칙 15)
- 투명도(alpha) 채널 선택 옵션
- `ColorPicker.Trigger` — 색상 스와치 트리거. `asChild`로 교체 가능
- `ColorPicker.Saturation` — 채도·명도 2D 선택 영역. `asChild`로 교체 가능
- `ColorPicker.HueSlider` — 색상(Hue) 슬라이더. `asChild`로 교체 가능
- `ColorPicker.AlphaSlider` — 투명도(Alpha) 슬라이더. `asChild`로 교체 가능
- `ColorPicker.Input` — HEX/RGB/HSL 텍스트 입력. `asChild`로 교체 가능
- `ColorPicker.Swatch` — 프리셋 색상 스와치 목록. `asChild`로 교체 가능
...
<ColorPicker value={color} onChange={setColor}>
  ...
  <ColorPicker.Input />
  <ColorPicker.Swatch colors={['#FF0000', '#00FF00', '#0000FF']} />
</ColorPicker>

// 트리거만 교체
<ColorPicker value={color} onChange={setColor}>
  <ColorPicker.Trigger asChild>
    <button style={{ background: color }} className="w-8 h-8 rounded-full" />
  </ColorPicker.Trigger>
</ColorPicker>
```
