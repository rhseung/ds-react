...
### 제네릭 compound component — 타입 안전한 선택

`CheckboxGroup`, `RadioGroup`, `Select` 등 다중 선택 컴포넌트는 render-prop children과 제네릭을 결합합니다. children을 함수로 선언하면 `T`로 좁혀진 서브컴포넌트를 받을 수 있고, 잘못된 value 전달이 컴파일 오류로 잡힙니다.

```typescript
// children 함수 — T로 좁혀진 서브컴포넌트 제공
...
정적 children(`<CheckboxGroup.Item />`)도 지원하며, 이때는 제네릭 제약 없이 사용 가능합니다.
...
> TextField의 에러 표시는 `TextField` 자체가 아닌 `Field`가 담당합니다. `TextField`는 시각 구조만 책임지며, 유효성 상태와 메시지는 외부 `Field`/`Form`에서 주입됩니다.
