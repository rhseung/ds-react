...
export function useIcon({ disabled = false }: UseIconOptions = {}): Store<{}> {
  const [disabledState, setDisabled] = useState(disabled);
  const { state: interaction, handlers } = useInteraction<SVGSVGElement>({
...
export type IconStore = ReturnType<typeof useIcon>;
